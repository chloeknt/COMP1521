...
static void execute_command(char **words, char **path, char **environment)
{
    ...
    if (strcmp(words[0], "date") == 0) {
        pid_t pid;
        extern char **environ;
        char *date_argv[] = {pathname, "--utc", NULL};


        if (posix_spawn(&pid, pathname, NULL, NULL, date_argv, environ) != 0) {
            perror("spawn");
            exit(1);
        }
    ...
    if (strcmp(words[0], "file") == 0) {
        pid_t pid;
        extern char **environ;
        char *file_argv[] = {pathname, words[1], NULL};


        if (posix_spawn(&pid, pathname, NULL, NULL, file_argv, environ) != 0) {
            perror("spawn");
            exit(1);
        }
    ...
    if (strcmp(words[0], "echo") == 0) {
        ...
        char *echo_argv[] = {pathname, echoline, NULL};


        if (posix_spawn(&pid, pathname, NULL, NULL, echo_argv, environ) != 0) {
            perror("spawn");
            exit(1);
        }
    ...
    // cp [filename src] [directoryname dest]
    if (strcmp(words[0], "cp") == 0) {
        pid_t pid;
        extern char **environ;
        /*// One source file one dest file
        char pathname2[PATH_MAX];
        char pathtoken[PATH_MAX];
        // Getting the current working directory 
        if (getcwd(pathtoken, sizeof pathtoken) == NULL) {
            perror("getcwd");
            return;
        }
        strcpy(pathname2, pathtoken);
        strcat(pathname2, "/");
        strcat(pathname2, words[1]);
        printf("%s\n", pathname2);*/
        ...
        //strcat(cpline, " -t ");
        //strcat(cpline, " ");
        //strcat(cpline, words[i]);
        //printf("%s\n", cpsrc);
        char *cp_argv[count + 1];
        cp_argv[0] = pathname;
        printf("%s\n", cp_argv[0]);
        ...
        if (posix_spawn(&pid, pathname, NULL, NULL, cp_argv, environ) != 0) {
            perror("spawn");
            exit(1);
        }
